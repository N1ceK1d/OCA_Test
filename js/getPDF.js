function generatePDF2(filename = 'Компания', format = 'PNG', quality = 1) {
    ...
    const elements = document.querySelectorAll('.diagramm-item');
...
function generateSolidPDF(diagramm_id, filename = 'Компания', format = 'PNG', quality = 1) {
    const pdf = new jsPDF('p', 'pt', 'a4');
    const pageHeight = pdf.internal.pageSize.getHeight();
    const pageWidth = pdf.internal.pageSize.getWidth();
    const margin = 10; // Отступ между блоками
    ...
    // Собираем все элементы с классом "employee-item"
    const element = document.getElementById(diagramm_id);
